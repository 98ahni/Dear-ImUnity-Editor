...
#define LabelLeft(label) {float indent = ImGui::GetCursorPosX(); std::string shownLabel(label, ImGui::FindRenderedTextEnd(label));if (!shownLabel.empty()) { ImGui::Text(shownLabel.c_str()); ImGui::SameLine(); ImGui::SetCursorPosX(indent + (ImGui::GetWindowWidth() < 384 ? 96 : ImGui::GetWindowWidth() * .25f)); }}
...
__declspec(dllexport) bool __stdcall ImGui_ShowStyleSelector314(const char* label)
{
	LabelLeft(label);
	FillWidth;
	return ImGui::ShowStyleSelector(HideLabel(label));
}
...
__declspec(dllexport) bool __stdcall ImGui_Selectable630(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size)
{
	LabelLeft(label);
	FillWidth;
	return ImGui::Selectable(HideLabel(label), selected, flags, size);
}
__declspec(dllexport) bool __stdcall ImGui_Selectable631(const char* label, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size)
{
	LabelLeft(label);
	FillWidth;
	return ImGui::Selectable(HideLabel(label), p_selected, flags, size);
}
...
__declspec(dllexport) bool __stdcall ImGui_BeginMenu667(const char* label, bool enabled)
{
	LabelLeft(label);
	FillWidth;
	return ImGui::BeginMenu(HideLabel(label), enabled);
}
...
__declspec(dllexport) bool __stdcall ImGui_MenuItem669(const char* label, const char* shortcut, bool selected, bool enabled)
{
	LabelLeft(label);
	FillWidth;
	return ImGui::MenuItem(HideLabel(label), shortcut, selected, enabled);
}
__declspec(dllexport) bool __stdcall ImGui_MenuItem670(const char* label, const char* shortcut, bool* p_selected, bool enabled)
{
	LabelLeft(label);
	FillWidth;
	return ImGui::MenuItem(HideLabel(label), shortcut, p_selected, enabled);
}
...
__declspec(dllexport) void __stdcall ImGui_TableSetupColumn769(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
	LabelLeft(label);
	FillWidth;
	ImGui::TableSetupColumn(HideLabel(label), flags, init_width_or_weight, user_id);
}
...
__declspec(dllexport) void __stdcall ImGui_TableHeader772(const char* label)
{
	LabelLeft(label);
	FillWidth;
	ImGui::TableHeader(HideLabel(label));
}
...
__declspec(dllexport) bool __stdcall ImGui_BeginTabItem804(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
	LabelLeft(label);
	FillWidth;
	return ImGui::BeginTabItem(HideLabel(label), p_open, flags);
}
...
__declspec(dllexport) bool __stdcall ImGui_TabItemButton806(const char* label, ImGuiTabItemFlags flags)
{
	LabelLeft(label);
	FillWidth;
	return ImGui::TabItemButton(HideLabel(label), flags);
}
...
__declspec(dllexport) int __stdcall ImGuiOnceUponAFrame_Get_RefFrame2267(ImGuiOnceUponAFrame* objectPtr)
{
	return objectPtr->RefFrame;
}
...
__declspec(dllexport) char* __stdcall ImGuiTextFilter_Get_InputBuf2292(ImGuiTextFilter* objectPtr)
{
	return objectPtr->InputBuf;
}
...
__declspec(dllexport) ImGuiID __stdcall ImGuiStorage_Get_key2331(ImGuiStorage* objectPtr)
{
	return 0;
}
__declspec(dllexport) void __stdcall ImGuiStorage_Set_key2331(ImGuiStorage* objectPtr, ImGuiID  value)
{
}
